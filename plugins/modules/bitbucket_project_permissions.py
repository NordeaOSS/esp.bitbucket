...
short_description: Manage Bitbucket project permissions
description:
...
options:
  ...
  permission:
    ...
    type: str
    choices: [ PROJECT_READ, PROJECT_WRITE, PROJECT_ADMIN, '' ]
    required: true
  validate_certs:
    description:
...
def grant_project_permissions(module, bitbucket, fail_when_not_exists=False, project_key=None, scope=None, name=None, permission=None):
    """
    Promote or demote either a user's or a group's permission level for the specified project
    scope: either 'users' or 'groups'.

    when fail_when_not_exists=False it just returns None and does not fail
    """
    info, content = bitbucket.request(
        api_url=(bitbucket.BITBUCKET_API_ENDPOINTS['projects-permissions'] + '/{scope}?name={name}&permission={permission}').format(
    ...
    if info['status'] == 401:
        module.fail_json(msg='The currently authenticated user is not a project administrator for {projectKey} project.'.format(
            projectKey=project_key,
        ))
    ...
    if info['status'] == 404:
        if fail_when_not_exists:
            module.fail_json(msg='The specified project or user or group does not exist')
        else:
            return None

    if info['status'] != 204:
        self.module.fail_json(msg='Failed to grant permission data which matches the supplied projectKey `{projectKey}`, `{name}` object and `{permission}` permission: {info}'.format(
            projectKey=project_key,
            name=name,
            permission=permission,
            info=info,
...
def revoke_project_permissions(module, bitbucket, fail_when_not_exists=False, project_key=None, scope=None, name=None):
    """
    Revoke all permissions for the specified project for a user or a group.
    scope: either 'users' or 'groups'.

    when fail_when_not_exists=False it just returns None and does not fail
    """
    info, content = bitbucket.request(
        api_url=(bitbucket.BITBUCKET_API_ENDPOINTS['projects-permissions'] + '/{scope}?name={name}').format(
            url=module.params['url'],
            projectKey=project_key,
            scope=scope,
            name=name,
    ...
    if info['status'] != 204:
        self.module.fail_json(msg='Failed to revoke permissions which matches the supplied projectKey `{projectKey}`, `{name}` object: {info}'.format(
            projectKey=project_key,
            name=name,
            info=info,
        ))
...
def main():
    ...
    argument_spec.update(
        ...
        permission=dict(type='str', no_log=False, choices=['PROJECT_READ', 'PROJECT_WRITE', 'PROJECT_ADMIN', '' ]),
    ...
    if not bitbucket.get_project_info(fail_when_not_exists=False, project_key=project_key):
        ...
        if module.params['user'] is not None:
            # Get a list of users that have been granted at least one permission for the specified project.
            users_with_access = bitbucket.get_project_permissions_info(fail_when_not_exists=False, project_key=project_key, scope='users', filter=None)
        ...
        if module.params['group'] is not None:
            # Get a list of groups that have been granted at least one permission for the specified project.
            groups_with_access = bitbucket.get_project_permissions_info(fail_when_not_exists=False, project_key=project_key, scope='groups', filter=None)
