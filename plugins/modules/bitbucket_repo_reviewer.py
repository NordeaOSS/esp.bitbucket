...
DOCUMENTATION = r'''
---
module: bitbucket_repo_reviewer
short_description: Manage default reviewer setting on repository level
description:
...
def add_default_repo_reviewer(module, bitbucket):
    ...
    info, content = bitbucket.request(
        api_url=bitbucket.BITBUCKET_API_ENDPOINTS['reviewers-repo'].format(
    ...
    if info['status'] == 401:
        module.fail_json(msg='The currently authenticated user has insufficient permissions to manage `{repositorySlug}` repository.'.format(
            repositorySlug=module.params['repository'],
        ))
...
def delete_default_repo_reviewer(module, bitbucket, reviewid):
    info, content = bitbucket.request(
        api_url=bitbucket.BITBUCKET_API_ENDPOINTS['reviewers-repo-delete'].format(
            url=module.params['url'],
            projectKey=module.params['project_key'],
            repositorySlug=module.params['repository'],
            id=reviewid
...
def main():
    ...
    result = dict(
        ...
        state=module.params['state'],
        approvals=module.params['approvals'],
        messages=[],
        json={},
    )
    ...
    existing_reviewers = bitbucket.get_repo_reviewers(fail_when_not_exists=False, filter=None)

    # Create new default reviewer in case it does not exist
    if state == 'present' and not any(
            d.get('targetRefMatcher', 'non_existing_reviewer_branch')['displayId'] == module.params['branch'] for d in existing_reviewers):
        if not module.check_mode:
            result['json'] = add_default_repo_reviewer(module, bitbucket)
        result['changed'] = True
    # Delete default repo reviewer
    elif existing_reviewers and (state == 'absent'):
        for d in existing_reviewers:
            if d.get('targetRefMatcher', 'non_existing_reviewer_branch')['displayId'] == module.params['branch']:
                if not module.check_mode:
                    result['json'] = delete_default_repo_reviewer(module, bitbucket, d.get('id'))
                result['changed'] = True
